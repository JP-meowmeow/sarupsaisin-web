...
function BookCard({ book }) {
  return (
    ...
    <div key={book.id} className="flex flex-col h-[100%] card card-compact bg-base-100 shadow-xl">
      ...
      <div className="card-body items-center text-center bg-gray-100">
        <h2 className="card-title text-xl sm:text-lg mb-2">{book.bookTitle}</h2>
        <p className="text-gray-600 line-clamp-2">{book.shortDescription}</p>
        <p className="text-xl font-bold mt-2">{book.price} บาท</p>
        <Link
          to={`/book/${book.id}`}
          className="btn btn-primary w-full mt-3 font-noto-sans-jp"
        >
          詳しく見る
        </Link>
      </div>
